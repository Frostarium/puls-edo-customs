-- Star Signis Blue - Rose
local s,id=GetID()
function s.initial_effect(c)
    --Add 1 "Star Signis" Spell/Trap from Deck to Hand (Ignition Version)
    ...
    --Add 1 "Star Signis" Spell/Trap from Deck to Hand (Quick Version)
    ...
    e7:SetCost(aux.SelfDiscardCost)
    e7:SetTarget(s.thtg)
    e7:SetOperation(s.thop)
    e7:SetCondition(s.quickcon)
    c:RegisterEffect(e7)

    --Return 1 Monster from GY to Deck, Special Summon this card from Hand (Ignition Version)
    ...
    --Return 1 Monster from GY to Deck, Special Summon this card from Hand (Quick Version)
...
function s.thfilter(c)
	return c:IsSetCard(0xbbb) and c:IsSpellTrap() and c:IsAbleToHand()
end
