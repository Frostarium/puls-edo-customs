...
function s.gyfilter(c,e,tp)
    if not (c:IsSetCard(0x657) and c:HasLevel() and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)) then return false end
    local g=Duel.GetFieldGroup(tp,LOCATION_MZONE,0)
    for tc in aux.Next(g) do
        if tc:HasLevel() and tc:GetLevel()==c:GetLevel() then return false end
    end
    return true
end
